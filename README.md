## Better Yandex
*UserCSS для Яндекса* [![Install directly with Stylus](https://img.shields.io/badge/Install%20directly%20with-Stylus-238b8b.svg)](//raw.githubusercontent.com/kodoomer/better-yandex/main/BetterYandex.user.css) 

Убирают рекламу и всякое ненужное. 
Имеет немного настроек.

> [!WARNING]
> Этот проект еще в частично активной разработке. Могут иметься недочеты.
> Если вы их заметете, пожалуйста напишите об этом в Issues

> [!IMPORTANT]
> Стиль сделан и работают только __на ПК__ и только на [беларусском Яндексе (.by)](//yandex.by), поскольку на нем не встроен ИИ на каждом углу. \
> Правила для расширения [Redirector](//einaregilsson.com/redirector) [тут](Redirector.json)

> [!NOTE]
> Стиль расчитан на то, что вы не вошли в аккаунт (и не планируете).
> Если вы вошли, некоторый фунционал будет недоступен.

На текущий момент поддерживаются следующие страницы:
- Главная страница ([/](//yandex.by/))
- Поиск сайтов ([/search](//yandex.by/search))
- Поиск картинок (и главная страница) ([/images*](//yandex.by/images))
- Поиск видео (и просмотр) ([/video/*](//yandex.by/video))
- Переводчик ([translate.yandex.COM](//translate.yandex.com))
- Погода ([/pogoda](//yandex.by/pogoda))
- Веб-кеш ([yandexwebcache.net/yandbtm](//yandexwebcache.net/yandbtm))
- Карты и метро ([/maps](//yandex.by/maps), [/metro](//yandex.by/metro))

#### todo
- [ ] поправить / добавить комметарии
- [ ] разделить метро, переводчик, погоду на несколько ссылок
- [ ] дописать ридми норамально
- [ ] нормальный темный режим для 404
